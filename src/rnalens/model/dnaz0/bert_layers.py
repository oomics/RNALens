...
flash_attn_qkvpacked_func = None # we disable flash_attn in favour of flash-attn from pytorch
...
class BertRegressionHead(nn.Module):
    def __init__(self, config, dropout_prob=0.2):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def forward(self, sequence_output: torch.Tensor) -> torch.Tensor:
        ...
        sequence_output = self.dense2(sequence_output)
...
class BertForPretraining(BertPreTrainedModel):
    """Pretraining rnalens with masked language modeling and minimum free energy prediction"""
    ...
    def __init__(self, config):
        ...
        self.mfe_pred = nn.Linear(config.hidden_size, 1)
        self.ss_cls = nn.Linear(config.hidden_size, 3)
    ...
    def forward(
        ...
        minimum_free_energies: Optional[torch.Tensor] = None, # mfe labels
        secondary_structure_labels: Optional[torch.Tensor] = None, # secondary structure labels
        ...
        mfe_prediction = self.mfe_pred(sequence_output[:,0,:])
        ss_prediction = self.ss_cls(sequence_output)
        ...
        if minimum_free_energies is not None:
            # when mfe == 1e-9, it actually means Nan
            # so we need to mask it out
            mfe_label_index = (minimum_free_energies != 1e-9)
            loss_mfe = nn.functional.mse_loss(
                mfe_prediction[mfe_label_index],
                minimum_free_energies[mfe_label_index].unsqueeze(1),
            )
            
            if torch.any(torch.isnan(loss_mfe)):
                # this is bcause sometimes there is no supervised loss
                loss_mfe = None
        if secondary_structure_labels is not None:
            loss_fct = nn.CrossEntropyLoss()
            loss_ss = loss_fct(ss_prediction.reshape(-1, 3),
                                 secondary_structure_labels.reshape(-1))
        
        
        if loss_mlm is not None:
            loss = loss_mlm
            if loss_mfe is not None:
                loss += loss_mfe
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.classifier = BertRegressionHead(config)
